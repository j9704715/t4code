...
export function isCodexCliVersionSupported(version: string) {
  return compareCodexCliVersions(version, MINIMUM_CODEX_CLI_VERSION) >= 0
}

export function formatCodexCliUpgradeMessage(version: string | null) {
  const versionLabel = version ? `v${version}` : "the installed version"
  return `Codex CLI ${versionLabel} is too old for T3 Code. Upgrade to v${MINIMUM_CODEX_CLI_VERSION} or newer and retry.`
}
